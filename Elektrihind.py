...
algAeg = datetime.utcnow().astimezone().replace(tzinfo=tz.tzutc()).astimezone(tz.gettz('Europe/Tallinn'))-timedelta(hours=1) #Lahutame ühe tunni, et hetke hinda ka näha
...
def elektriHindVahemikus(algKuupäev, lõppKuupäev, apiAadress):
    ...
    if APIvastus["success"]:
        ...
        joonesta.bar(list(elektriHinnad.keys()),list(elektriHinnad.values()), width=0.03, color='royalblue')
        joonesta.title("Elektrihind vahemikus "+list(elektriHinnad.keys())[0].strftime("%d.%m.%Y(%H:%M)")+" - "+list(elektriHinnad.keys())[-1].strftime("%d.%m.%Y(%H:%M)"))
        joonesta.ylabel("¢/kWh")
